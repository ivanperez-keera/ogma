...
- A state machine diagram that describres the state machine that the system
  under observation should follow.

- An execution mode that determines if the cFS application should publish the
  expected state machines, check the modeled machine state against a known
system state, or determine which states the system can legally transition to.
...
For the sake of the example, we assume that the system state is published as a
`uint8_t` to the message `STATE_MID`, which is a label for the ID `0x1878`, and
the input number is published as an `int32_t` to the message `SAMPLE_MID`,
which is a label for the ID `0x1879`.
...
generated by Ogma's application, and only notify when the system transitions
illegally.
