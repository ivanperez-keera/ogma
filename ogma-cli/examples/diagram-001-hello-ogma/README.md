...
The first element to provide is the states that the resulting system can be in.

In our example, that information is provided in a `diagram-copilot.dot` file,
which, in our case, looks like the following (this does not include expressions
or constraints for the transitions, which are described later):
...
variable `input` becomes greater than 180, and to state `2` when it becomes
smaller than 120. In either case, if the negation of that condition becomes
true after transitioning, then the system transitions back to the initial
state, but it does not transition directly between `1` and `2`.
...
To build the application, we must first go to the directory where the
...
The code generated expects the state machine to start at the initial state `0`,
the current system input to be available in a global variable called `input`,
and an existing function `handler` that handles notifications from the
generated code with the new state. In `check` mode, the generated code also
expects a variable `state` that holds the system state, which the generated
monitor compares against the internally calculated or expected state. The names
`input` and `state` can be customized by the user via a combination of
command-line flags and transition expressions.  Run `ogma diagram --help` for
more information.
...
that defines different values for `input`, calls the state machine
implementation to calculate the new expected state, and reports the results:
...
The generated state machine implementation transitions between the
