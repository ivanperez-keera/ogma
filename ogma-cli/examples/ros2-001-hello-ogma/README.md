...
- The variable `input_value` can be used as an input variable in expression.
...
That starts the Docker container and opens a bash terminal running as the
...
Finally, we show how the generated application is able to read new values
...
`input_value`, and the bottom pane shows the output messages published by the
